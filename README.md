...
#### 1. Create Development Environment Instance
  * Create new development environment (https://aws.amazon.com).
...
```
 $ ssh -i ~/.ssh/udacity_key.rsa grader@13.233.89.95 -p 2200
```
...
#### 15. Clone the Catalog app from Github
  ...
  * Clone the **Item-Catalog** to the catalog directory:
    ...
    ```
     $ git clone https://github.com/khushkoolkhosla/Item-Catalog/.git 
    ```

  * Make a project.wsgi file to serve the application over the mod_wsgi. with content:

    ```
     $ touch project.wsgi && nano project.wsgi
    ```
    ...
    ```
  * Inside *project.py*, *database_setup.py*, *menus.py*  database connection is now performed with:
...
#### 16. Edit the default Virtual File with following content:
  ...
  ```
  <VirtualHost *:80>
    ServerName 13.233.89.95
    ServerAdmin khushkoolkhosla2697@@gmail.com
    WSGIScriptAlias / /var/www/catalog/Item-Catalog/project.wsgi
